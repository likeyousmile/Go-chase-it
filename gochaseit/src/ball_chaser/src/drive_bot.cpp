    ...
    ball_chaser::DriveToTarget::Response& res)
{
    ...
    ROS_INFO("DriveToTargetRequest received - linear_velocity:%1.2f, angular_velocity:%1.2f", linear_x, angular_z);


    // Publish velocity to the car  
    ...
    // Wait 3 seconds for car to settle
    //(deleted the settle duration based on the feedback
    //ros::Duration(3).sleep();

    // Return a response message
    res.msg_feedback = "Velocity set - linear " + std::to_string(linear_x) + " , angular: " + std::to_string(angular_z);
